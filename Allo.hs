...
import Data.List ( tails, inits, insert, (\\), isInfixOf, maximumBy , sortBy)
...
apply pat word = nub [ simp (remove is word) | is <- matches pat word ]
 where
  simp ('+':'+':s) = simp ('+':s)
  simp (c:s)       = c : simp s
  simp ""          = ""
  
  remove is s = [ if i `elem` is then '+' else c | (c,i) <- s `zip` [0..] ]

applyTable :: [[String]] -> String -> [String]
